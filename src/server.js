...
if (!process.env.FACE_TEMPLATE_SECRET) console.error('[FACE] FACE_TEMPLATE_SECRET is not configured; face auth disabled');
const FACE_TEMPLATE_KEY = crypto.createHash('sha256').update(String(process.env.FACE_TEMPLATE_SECRET || '')).digest();
const FACE_COMMANDS = ['blink','turn_left','turn_right','open_mouth','nod'];
const FACE_ALLOWED_ORIGIN = process.env.FACE_ALLOWED_ORIGIN || process.env.CORS_ORIGIN || '';
...
function faceIp(req){ return req.headers['x-forwarded-for']?.split(',')[0]?.trim() || req.socket.remoteAddress || ''; }
function faceOriginGuard(req,res,next){ const origin=req.get('origin'); if(origin && FACE_ALLOWED_ORIGIN && origin!==FACE_ALLOWED_ORIGIN) return res.status(403).json({erro:'Origem não autorizada'}); next(); }
...
function faceKeyReady(){ return Boolean(process.env.FACE_TEMPLATE_SECRET); }
function encryptFaceTemplate(value){ if(!faceKeyReady()) throw new Error('FACE_TEMPLATE_SECRET missing'); const iv=crypto.randomBytes(12), c=crypto.createCipheriv('aes-256-gcm',FACE_TEMPLATE_KEY,iv); const out=Buffer.concat([c.update(Buffer.from(JSON.stringify(value))),c.final()]); return {ciphertext:out.toString('base64'),iv:iv.toString('base64'),tag:c.getAuthTag().toString('base64')}; }
...
async function faceChallenge(req,purpose,adminId){ const raw=crypto.randomBytes(32).toString('hex'), hash=crypto.createHash('sha256').update(raw).digest('hex'); const command=FACE_COMMANDS[crypto.randomInt(FACE_COMMANDS.length)]; const q=await pool.query("INSERT INTO master_face_challenges(challenge_hash,admin_id,purpose,command,expires_at,request_ip) VALUES($1,$2,$3,$4,NOW()+INTERVAL '5 minutes',$5) RETURNING id",[hash,adminId||null,purpose,command,faceIp(req)]); return {challenge:raw,challenge_id:q.rows[0].id,command}; }
...
async function useFaceChallenge(raw,purpose){ const h=crypto.createHash('sha256').update(String(raw||'')).digest('hex'); const q=await pool.query("UPDATE master_face_challenges SET used_at=NOW() WHERE challenge_hash=$1 AND purpose=$2 AND used_at IS NULL AND expires_at>NOW() RETURNING id,admin_id,command",[h,purpose]); return q.rows[0]||null; }
