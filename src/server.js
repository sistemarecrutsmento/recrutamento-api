...
app.post('/api/video/preview-login', async (req, res) => {
  ...
  try {
    const company = await pool.query(`SELECT id FROM empresas WHERE nome='Preview Synthetic Company' ORDER BY id DESC LIMIT 1`);
...
app.post('/api/candidato/candidatar/:vagaId', authCandidato, async (req, res) => {
  ...
  try {
    // A inscrição já foi concluída pelo candidato. A candidatura começa na etapa 1 (Triagem);
    // a etapa 0 (Inscrição) permanece registrada no histórico como concluída.
    const { rows } = await pool.query(
